...
#ifndef MECANUM_DRIVE_ODOM_PUBLISHER__ODOMETRY_HPP_
#define MECANUM_DRIVE_ODOM_PUBLISHER__ODOMETRY_HPP_
...
public:
  ...
  bool update(double wheel_front_left_vel, double wheel_back_left_vel, double wheel_back_right_vel,
              double wheel_front_right_vel, const double dt);

  /// \return position (x component) [m]
  double getX() const
  {
    return position_x_in_base_frame_;
  }
  /// \return position (y component) [m]
  double getY() const
  {
    return position_y_in_base_frame_;
  }
  /// \return orientation (z component) [m]
  double getRz() const
  {
    return orientation_z_in_base_frame_;
  }
  /// \return body velocity of the base frame (linear x component) [m/s]
  double getVx() const
  {
    return velocity_in_base_frame_linear_x;
  }
  /// \return body velocity of the base frame (linear y component) [m/s]
  double getVy() const
  {
    return velocity_in_base_frame_linear_y;
  }
...
#endif  // MECANUM_DRIVE_ODOM_PUBLISHER__ODOMETRY_HPP_ */
