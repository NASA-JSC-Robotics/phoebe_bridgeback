...
TEST_F(MecanumDriveControllerTest,
  ...
  ASSERT_EQ(controller_->params_.kinematics
...
TEST_F(MecanumDriveControllerTest,
  ...
  for (size_t i = 0; i < reference_interface_names.size(); ++i) {
    const std::string ref_itf_name =
        std::string(controller_->get_node()->get_name()) + std::string("/") +
        reference_interface_names[i];
    EXPECT_EQ(reference_interfaces[i].get_name(), ref_itf_name);
    EXPECT_EQ(reference_interfaces[i].get_prefix_name(),
...
TEST_F(
  ...
  ASSERT_EQ(controller_->on_configure(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_EQ(controller_->command_interfaces_[NR_CMD_ITFS - 4].get_value(),
            101.101);
  ASSERT_EQ(controller_->on_deactivate(rclcpp_lifecycle::State()),
            NODE_SUCCESS);
  ASSERT_TRUE(std::isnan(
      controller_->command_interfaces_[NR_CMD_ITFS - 4].get_value()));
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_TRUE(std::isnan(
      controller_->command_interfaces_[NR_CMD_ITFS - 4].get_value()));
...
// when update is called expect the previously set reference before calling
// update, inside the controller state message
TEST_F(MecanumDriveControllerTest,
...
// reference_interfaces and command_interfaces values depend on the
// reference_msg, the below test shows two cases when reference_msg is not
// received and when it is received.
TEST_F(MecanumDriveControllerTest,
  ...
  // reference_msg is published with provided time stamp when publish_commands(
...
TEST_F(
  ...
  EXPECT_NE((*(controller_->input_ref_.readFromNonRT()))->header.stamp.sec,
            0.0);
}

// when the reference_msg has valid timestamp then the timeout check in
// reference_callback() shall pass and reference_msg is not reset
TEST_F(MecanumDriveControllerTest,
...
// when in chained mode the reference_interfaces of chained controller and
// command_interfaces of preceding controller point to same memory location,
// hence reference_interfaces are not exclusively set by the update method of
// chained controller
TEST_F(
    MecanumDriveControllerTest,
  ...
  // set command statically
  joint_command_values_[1] = command_lin_x;
  // imitating preceding controllers command_interfaces setting
  // reference_interfaces of chained controller.
...
// when ref_timeout = 0 expect reference_msg is accepted only once and
// command_interfaces are calculated to valid values and reference_interfaces
// are unset
TEST_F(
    MecanumDriveControllerTest,
...
TEST_F(
  ...
  EXPECT_TRUE(
      std::isnan((*(controller_->input_ref_.readFromNonRT()))->twist.linear.x));
  EXPECT_TRUE(
      std::isnan((*(controller_->input_ref_.readFromNonRT()))->twist.linear.y));
  EXPECT_TRUE(std::isnan(
      (*(controller_->input_ref_.readFromNonRT()))->twist.angular.z));
  controller_->ref_timeout_ = rclcpp::Duration::from_seconds(0.0);

  // reference_callback() is called implicitly when publish_commands() is
  // called. reference_msg is published with provided time stamp when
  // publish_commands( time_stamp) is called
  publish_commands(controller_->get_node()->now());
