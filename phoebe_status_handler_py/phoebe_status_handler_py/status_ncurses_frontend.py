...
class WheelStringObject(StringObject):
...
class StatusNcursesFrontend(object):
    ...
    def __init__(self):
        ...
        self.status_objects = {
            "leftRear":   WheelStringObject(self.screen_, self.BODY_START_ROW-1, self.BODY_START_COL-1),
            "leftFront":  WheelStringObject(self.screen_, self.BODY_START_ROW-1, self.BODY_END_COL-2),
            "rightRear":  WheelStringObject(self.screen_, self.BODY_END_ROW,   self.BODY_START_COL-1),
            "rightFront": WheelStringObject(self.screen_, self.BODY_END_ROW,   self.BODY_END_COL-2),
            "battery":    StringObject(self.screen_, self.BODY_START_ROW+1, self.BODY_START_COL+2, " BAT "),
            "charging":   StringObject(self.screen_, self.BODY_START_ROW+3, self.BODY_START_COL+2, " CHG "),
        }
    ...
    def update_all_corner_lighting(self, upper_color, lower_color=None, is_blinking=False, content=" "):
        self.update_corner_lighting("leftRear",   upper_color, lower_color, is_blinking, content)
        self.update_corner_lighting("leftFront",  upper_color, lower_color, is_blinking, content)
        self.update_corner_lighting("rightRear",  upper_color, lower_color, is_blinking, content)
        self.update_corner_lighting("rightFront", upper_color, lower_color, is_blinking, content)
    ...
    def update(self, state: StatusState):
        ...
        if state.estop_state == StatusState.ROBOT_STATE_OFF:
            ...
            self.update_all_corner_lighting("red", is_blinking=True, content="R")
        ...
        if state.battery_state == StatusState.BATTERY_STATE_LOW:
            self.update_battery("yellow", is_blinking=False)
